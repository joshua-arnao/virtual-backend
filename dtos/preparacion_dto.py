...
class PreparacionResponseDTO(validador.SQLAlchemyAutoSchema):
    # Nested => es un tipo de campo que sirve para relacionar un DTO con otro DTO y usamos el parametro nested para indicar a que DTO nos queremos relacionar, tiene que ser el mismo nombre que el relationship pero si quisieramos tener un nombre diferente entonces agregamos el parametro data_key en el cual indicaremos como se llamará nuestra llave en el resultado pero de igual forma tendremos que utilizar el nombre de nuestro relationship como atributo de la clase
    receta = fields.Nested(nested=RecetaResponseDTO, data_key= 'receta_relacion')
    class Meta:
        model = Preparacion
        # load_instance => Cargara las instancias relacionadas al modelo preparación
        load_instance = True
        # Si queremos que el DTO tambien valide el tipo de dato y algunas propuedas que le hemos dado a nuectra relación entonces tedremos que agregar el atributo include_fk para que se inclya las validaciones
        include_fk = True
